...
<div class="form-group">
    ...
    <select class="form-control select2" style="width:100%" name="recipient" id="recipient" required>
        <option selected disabled>-- Select one --</option>
        @foreach(\App\Models\Policy::getRecipientSelectOptions() as $recipient)
            @if($policy->recipient == $recipient['value'])
                <option value="{{ $recipient['value'] }}" selected>{{ $recipient['label'] }}</option>
            @else
